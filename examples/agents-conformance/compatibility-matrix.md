...
This fixture is the pinned, source-unmodified compatibility seam extended through tickets 02–11.
...
| celld loaded-isolate host authority and stream ownership | in-crate `celld` tests exercise raw storage/SQL/alarm/DO/service/RPC/loader denials and exact stream owners | adapted | this invariant is unreachable through the public fixture routes; tests use real V8 loaded workers and unique stream registries rather than source inspection |
| celld host-isolate slot re-entry and shutdown drain | concurrent in-crate slot tests return each Agent marker; Compose `shutdown` sends SIGTERM during a hanging loaded call | adapted | the slot test asserts classified `host_cell_lost` on a freed isolate and the process helper requires a bounded clean exit; Compose state is disposable |
