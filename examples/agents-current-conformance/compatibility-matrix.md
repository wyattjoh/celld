...
| Agent state and application SQL persistence | `/current/state/<name>` remains isolated after inactivity and reconnect | adapted | `setState` and `this.sql` use the owning cell's authoritative SQLite database |
...
| interrupted-stream resumption, tools, approvals, facets, and MCP | not exercised by this chat slice | unsupported | follow-up tickets must add public-boundary evidence before support is claimed |
