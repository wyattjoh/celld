...
export function checkCompatibility(root = DEFAULT_ROOT) {
  ...
  const matrix = readFileSync(join(root, target.matrix), "utf8");
  ...
  for (const [name, expected] of Object.entries(target.packages)) {
    ...
    assertEqual(lockRoot[lockSection][name], expected.version, `${name} lockfile root version`, "lock-version");
  ...
  for (const status of MATRIX_STATUSES) {
    if (!new RegExp(`\\|\\s*${status}\\s*\\|`, "i").test(matrix)) {
      fail("matrix-status", `${target.matrix} does not contain a ${status} row`);
    }
  }
  for (const [name, expected] of Object.entries(target.packages)) {
    if (!matrix.includes(`${name}@${expected.version}`)) {
      fail("matrix-version", `${target.matrix} does not record ${name}@${expected.version}`);
    }
  }
  ...
  if (source.includes('from "@cloudflare/agents"') || source.includes("./vendor/") || source.includes("./patched/")) {
    fail("source", "index.js must not use the legacy SDK or an application-specific patch");
  }
